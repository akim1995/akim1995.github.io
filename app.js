// elm-watch hot {"version":"1.1.2","targetName":"defaultTarget","webSocketPort":39175}
"use strict";
(() => {
...
function _Platform_initialize(programType, isDebug, debugMetadata, flagDecoder, args, init, impl, stepperBuilder)
{
	...
	function setUpdateAndSubscriptions() {
		update = impl.aY || impl._impl.aY;
		subscriptions = impl.aU || impl._impl.aU;
		if (isDebug) {
	...
	return _Platform_initialize(
		...
		function(sendToApp, initialModel) {
			...
			return _Browser_makeAnimator(initialModel, function(model)
			{
				...
				var nextNode = impl.aZ(model); // added by elm-watch
	...
	return _Platform_initialize(
		...
		function(sendToApp, initialModel) {
			...
			return _Browser_makeAnimator(initialModel, function(model)
			{
				...
				var doc = impl.aZ(model); // added by elm-watch
...
function _Browser_application(impl)
{
	...
	var key = function() { key.a(impl.aJ(_Browser_getUrl())); }; // added by elm-watch
	...
	return _Browser_document({
		...
		aH: function(flags)
		{
			...
			return A3(impl.aH, flags, globalThis.__ELM_WATCH.INIT_URL, key); // added by elm-watch
		},
		...
		aZ: function(model) { return impl.aZ(model); }, // added by elm-watch
...
var $author$project$Types$Msg$LinkClicked = function (a) {
	return {$: 0, a: a};
};
var $author$project$Types$Msg$UrlChanged = function (a) {
	return {$: 1, a: a};
};
		...
		[
			...
			_List_fromArray(
				[
					...
					_List_fromArray(
						[
							$elm$html$Html$text('Web Developer JS/TS/Node. Haskell and Elm as a hobby')
						]))
				]))
		]));
...
var $author$project$Main$homePageView = A2(
	...
	_List_fromArray(
		[
			...
			_List_fromArray(
				[
					$elm$html$Html$text('Additionally, you can download my CV as a '),
					A2(
					$elm$html$Html$a,
					_List_fromArray(
						[
							$elm$html$Html$Attributes$href('https://akim1995.github.io/cv/cv.pdf'),
							$elm$html$Html$Attributes$target('_blank')
						]),
					_List_fromArray(
						[
							$elm$html$Html$text('PDF file')
						])),
					$elm$html$Html$text('. ')
				]))
		]));
